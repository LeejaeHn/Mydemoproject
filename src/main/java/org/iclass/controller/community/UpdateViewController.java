	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//지정된 idx 메인글 가져오기
				...
				try {
					...
					//2) idx 글 작성자와 로그인 작성자 비교 - 일치하지 않으면 예외 발생(url에서 임의로 idx를 보낼 수 있기 때문)
					//	인가(권한) 확인, 인증(로그인-사용자 확인)
				if(vo==null || !vo.getWriter().equals(user.getUserid())) throw new RuntimeException();	//예외 발생시키기
