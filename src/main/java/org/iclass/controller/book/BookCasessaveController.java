...
@Slf4j
public class BookCasessaveController implements Controller {

	@Override
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//사용자가 업로드한 파일이 저장될 서버 경로(파일시스템 경로) 
	      String path = "D:\\Class231228-001\\upload";
	      //서버의 로컬 경로에 대해 url  을 지정합니다 -> 톰캣 server.xml
	      // <Context dcoBase="D:/Class231228-001/upload" path="/upload"/> 추가하기
	      
	      // 업로드 가능한 최대 크기(바이트)
	      int maxSize = 10*1024*1024;      //1024바이트=1KB, 10MByte로 설정
	      
	      //request 를 파일을 받을 수 있는 multipartRequest로 사용해야 합니다. : cos 라이브러리 추가 
	      ...
	      //기존 방식처럼 text 타입 등 파라미터 받아오기 - 문자열 파라미터도 MultipartRequest 를 사용
	      ...
	      // 서버의 파일 시스템 D:\\Class231228-001\\upload 경로에서 파일이 생겼는지 확인하기
	      ...
	      try {
	    	  ...
	    	  if(result!=1) {
	    		  //todo : 이전 예제 참고해서 등록 실행하면 쿠키 이용해서 메세지를 전달하기
	    		  url="new";
	    	  }
	      }catch (Exception e) {
